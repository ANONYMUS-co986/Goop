// midshot.js <url> <out.png> <scrollY> [postScrollMs] — mid-animation receipts capture
...
(async () => {
  ...
  const out = process.argv[3] || 'mid.png';
  const scrollY = parseInt(process.argv[4] || '0', 10);
  const postMs = parseInt(process.argv[5] || '320', 10);
  const waitMs = 4000;
  const waitFor = '';
  const clickSel = '';
  const hoverSel = '';
  ...
  const slow = parseFloat(process.env.QA_SLOW || '1');
  if (slow > 1) {
    await page.evaluate((k) => {
      const g = window.__gsap || (window.gsap);
      if (g && g.globalTimeline) g.globalTimeline.timeScale(1 / k);
    }, slow).catch(() => {});
  }
  let skipped = false;
