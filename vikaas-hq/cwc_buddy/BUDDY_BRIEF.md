...
Upload **`art/rebee_hero_SCRAPSCAN_v2.png`** — **the bonus-point concept**: ReBee with his SCRAP-SCAN superpower ACTIVATED. A bright emerald scan beam from his visor hits a dead phone and a big glowing hologram explodes it into its hidden value — copper coil, gold chip, battery, coins, a recycler location pin. It LITERALLY pictures the Q3 answer, so Q3+Q4 read as one cohesive, unforgettable concept (the exact thing "most creative" juries reward). Portrait 896×1200, generated with a hard no-text constraint so the hologram stays clean. **Fallback = `art/rebee_hero_TRIUMPH.png`** (hero fist-raised on the e-waste mountain at sunrise, cape flying, zero text — always safe). Same ReBee character in both.
