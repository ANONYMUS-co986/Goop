...
Upload **`art/rebee_hero_v6_FINAL.png`** — WON the 3-way playoff: superhero origin moment, rising from the junk drawer with parts magnetically assembling. Why it beats the others: (1) standard superhero shot would show a city — THIS pictures the PROBLEM (dead phones, tangled cables, one cracked screen) = wordless theme-match to the brief; (2) round bee body stays character-consistent with the poster/action shots; (3) Q2 text says "hatched from an electronics drawer in Gurugram" — this IS that moment; (4) one eye powering on = "just born" narrative charm.
- **v6 FINAL (11 Aug, research-backed):** `art/rebee_hero_v6_FINAL.png` — drawer-awakening scene (e-waste drawer → ReBee rising, both green eyes blazing, Indian girl watching in wonder, warm morning light). Selected by rubric research (theme relevance / emotional storytelling / focal point / character consistency) + pixel QA. v3 stays the fallback; alternates: v5_doorstep_dawn, v5_guardian_skyline, v4_contender. Before/after: `art/REBEE_FINAL_SHOWDOWN.png`.
