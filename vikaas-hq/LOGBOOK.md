...
## V11 — JUICE PASS + the great streaming fix (2026-08-06)
- USER BUG REPORT: videos didn't play + site lacked anims. Both fixed same turn.
- **ROOT CAUSE of dead videos: python http.server sends full 200, no Range support → Chrome/Safari refuse <video>.** Wrote `vikaas-hq/serve.py` (Range-capable: 206 Partial Content, Accept-Ranges, suffix-ranges, threading, CORS, no-store). Verified: curl Range → 206 + Content-Range; uicheck sees 10/10 mp4 responses 206. Preview server restarted on it (id vikaas-portfolio-63a70c6b…).
- JUICE shipped: `portfolio/assets/site.js` (self-injected on all pages): burger topbar + 6-room overlay (staggered reveal, YOU ARE HERE pill, Esc/close), custom cursor + data-cursor labels, data-tip tooltip engine (clamped), .rv reveal-on-scroll (io-staggered), page-wipe acid transitions between rooms, magnetic chips, count-up-on-view, film grain layer, prefers-reduced-motion + touch fallbacks everywhere. site.css component layer (vbar/vnav/vtip/vwipe/cur/ticker/spin/bootbar).
- Index: ticker strip, boot progress bar, chip magnets+tooltips, rooms rv-stagger + cursor labels. Ledger: stamps self-explain via tips, big-nums count-up. Films: IO auto-preview + PLAY WITH SOUND (unmute+controls+restart) + spinner/error states.
- QA rig `engine/uicheck.js`: drives LIVE server, does real interactions, asserts + console-error capture + screenshots. Bugs it caught: (1) ₹ chip double-<b> → undefined data-n TypeError that killed inline IIFE (had silently eaten all card handlers + mobile errors!) (2) test-selector false-positive (10 snd = buttons+chips). Final: **20/20 PASS** incl. mobile clean.
- a11y/perf guard held: reduced-motion disables cursor/wipe/counters, transform-only anims.
